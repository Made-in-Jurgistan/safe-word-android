...
<div style="background: linear-gradient(135deg, #080c22 0%, #111740 40%, #0d1233 100%); border-radius: 14px; padding: 44px 32px 30px; text-align: center; margin-bottom: 40px; border: 1px solid rgba(0, 71, 171, 0.25); box-shadow: 0 4px 24px rgba(0, 0, 0, 0.3);">
  <div style="width: 180px; margin: 0 auto; text-align: left;">
    <div style="text-align: center;">
      <img src="app/src/main/res/drawable-nodpi/safeword_icon.png" alt="Safe Word Logo" style="width: 56px; height: 56px; opacity: 0.92;" />
    </div>
    <div style="color: #0047AB; font-size: 10.5px; font-style: italic; letter-spacing: 0.8px;">Made in Jurgistan</div>
  </div>
  <p style="color: #8892b0; font-size: 17px; margin: 22px 0 14px; font-weight: 300;">Complete Voice Commands Reference</p>
  <p style="color: #4a5580; font-size: 12px; margin: 0; letter-spacing: 1px; text-transform: uppercase;">Version 1.4 &nbsp;·&nbsp; August 2026 &nbsp;·&nbsp; On-Device Voice-to-Text Overlay</p>
</div>
...
3. [Voice Commands](#voice-commands) — Deletion · Undo & Redo · Selection · Word Recognition · Clipboard · Replace & Switch · Search · Navigation · Formatting
4. [Draft Transcript Editing](#draft-transcript-editing)
5. [Custom Voice Commands](#custom-voice-commands)
6. [Semantic Understanding](#semantic-understanding)
7. [Spelling Mode & Letters](#spelling-mode--letters)
8. [Spoken Emoji](#spoken-emoji)
9. [Spoken Punctuation & Spacing](#spoken-punctuation--spacing)
10. [Number Words & Ordinals](#number-words--ordinals)
11. [Fractions](#fractions)
...
| **Command Mode** | Optional | `command mode` or `dictation command` — forces command detection |
...
|---------|---|---|
| **stop listening** | stop recording, stop, stop now, stop dictation, end dictation, end recording, finish dictation, finish, finished, finish that, done, that's it, that's all, commit, commit that, commit it, commit this, commit now | Stop voice recording session and commit transcript |
| **send** | send that, send message, send it, send now, send the message | Send current message (if supported by app) |
| **submit** | submit that, submit it, submit form, confirm, confirm it | Submit form or dialog |
| **clear** | clear all, clear everything, clear the field, erase all, erase everything, wipe everything, start over | Clear all text from field |
| **go back** | navigate back, go back a page | Navigate to previous screen |
| **open settings** | show settings, dictation settings, safe word settings, open preferences | Open app settings |
| **scroll up** | page up | Scroll document up one page |
| **scroll down** | page down | Scroll document down one page |
| **dismiss** | dismiss keyboard, hide keyboard, hide the keyboard, close keyboard | Hide onscreen keyboard |
...
| Command | Alternatives | Action |
|---------|---|---|
| **delete that** | delete this, delete it, erase that, erase this, erase it, remove that, remove this, remove it | Delete currently selected text |
| **kill it** | kill that, wipe that, trash that, scrap that, drop that | Delete currently selected text (informal) |
| **delete last word** | delete the last word, erase last word, erase the last word, remove last word, remove the last word, delete previous word | Delete the word before cursor |
| **delete last sentence** | delete the last sentence, delete sentence, erase last sentence, erase the last sentence | Delete the previous sentence |
| **backspace** | hit backspace, press backspace | Delete one character before cursor |
| **delete last paragraph** | delete the last paragraph, erase last paragraph, erase the last paragraph | Delete the previous paragraph |
...
| Command | Alternatives | Action |
|---------|---|---|
| **undo** | undo that, undo it, undo last, undo last action, scratch that, take that back, revert, revert that, reverse that | Undo the last action |
| **redo** | redo that, redo it, restore, restore that | Redo the last undone action |
...
| **swap [X] and [Y]** | "swap hello and world" | Find X and replace with Y |
...
| **find [X] and replace by [Y]** | "find error and replace by fix" | Find X and replace with Y |
...
<div style="background: #fef9f0; border-left: 4px solid #f0a030; border-radius: 0 8px 8px 0; padding: 10px 16px; margin: 12px 0; font-size: 14px;">
<strong>🎯 ASR-tolerant verbs:</strong> If the speech model mis-transcribes the verb, common substitutions are still recognised. Replace verbs: <code>replace</code>, <code>place</code>, <code>replaces</code>, <code>replays</code>, <code>displace</code>. Swap verbs: <code>swap</code>, <code>swat</code>, <code>swaps</code>, <code>switch</code>, <code>switches</code>, <code>switched</code>. Connectors: <code>with</code>, <code>to</code>, <code>and</code>, <code>for</code>, <code>by</code>, <code>into</code>. So <code>"place hello with world"</code>, <code>"swaps hello and world"</code>, and <code>"switched old to new"</code> all map to the same replace action.
</div>

### Search

Search the web or look up information. Searches open in the default browser or search app.

| Command | Alternatives | Action |
|---------|---|---|
| **search for [query]** | — | Search the web for the query |
| **look up [query]** | — | Look up the query online |
| **google [query]** | — | Search Google for the query |
| **search [query]** | — | Search the web for the query |

<div style="background: #f0f7ff; border-left: 4px solid #0047AB; border-radius: 0 8px 8px 0; padding: 10px 16px; margin: 12px 0; font-size: 14px;">
<strong>📌 Note:</strong> Search commands are only available in text fields (not search bars, to avoid conflicts with the field's own search function).
</div>
...
| Command | Alternatives | Action |
|---------|---|---|
| **new line** | newline, next line, line break, linefeed, line feed, break line, go to next line, enter, press enter, hit enter | Insert line break (Enter) |
| **new paragraph** | next paragraph, start new paragraph, break paragraph | Insert paragraph break (double Enter) |
...
## Draft Transcript Editing

Safe Word displays a live, editable draft of your transcription in the floating overlay while you dictate. This lets you review and correct text before it's committed to the target app.

### How It Works

<div style="background: #f0f7ff; border: 1px solid #cce0ff; border-radius: 8px; padding: 16px 20px; margin: 12px 0;">

1. **Start dictation** — Tap the floating mic button. The overlay expands to show a live transcript preview.
2. **Watch the draft** — As you speak, transcribed text appears in real time in the overlay's text field.
3. **Edit at any time** — Tap the text field to correct mis-transcriptions, add punctuation, or modify content. The draft updates live.
4. **Commit** — Say `"stop listening"` (or any stop variant) or tap the stop button. The final draft text is inserted into the target app.
...
| Feature | Description |
|---------|-------------|
| **Live preview** | Streaming transcription appears in the overlay as you speak |
| **Full editability** | Tap the text field to type corrections at any point during dictation |
| **Word counter** | A live word count is displayed below the draft |
| **Clear draft** | A clear button lets you wipe the draft and start over |
| **Single commit** | Text is inserted into the target app only once, when you stop — not line-by-line during streaming |
| **Auto-learn corrections** | Edits you make to the draft are learned as personalised dictionary entries for future sessions |

<div style="background: #fef9f0; border-left: 4px solid #f0a030; border-radius: 0 8px 8px 0; padding: 10px 16px; margin: 12px 0; font-size: 14px;">
<strong>💡 Tip:</strong> If Moonshine mis-transcribes a word, just tap the draft and fix it. Safe Word automatically learns the correction and will apply it in future dictation sessions.
</div>
...
| "reverse that" | Undo | Undoes last action |
...
| "delete the last character" | Backspace | Deletes one character |
| "hide the keyboard" | Dismiss | Dismisses onscreen keyboard |
| "open up preferences" | OpenSettings | Opens settings |
...
When the speech model splits a short command across two separate lines (e.g. a brief pause between <em>scratch</em> and <em>that</em>), Safe Word automatically joins the fragments and recognises the combined command. The previously-inserted fragment is retracted from the draft before the command fires — so <code>"scratch"</code> + <code>"that"</code> still triggers **Undo** without leaving the word <em>scratch</em> in your text.
...
<div style="text-align: center; padding: 32px 16px 8px; color: #5a6690; font-size: 12px;">
  ...
  <span style="color: #0047AB;">© 2026 Made in Jurgistan</span>
</div>
