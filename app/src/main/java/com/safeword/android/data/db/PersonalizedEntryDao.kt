...
interface PersonalizedEntryDao {
    ...
    /**
     * Insert a fresh entry. Returns -1 when the unique index on
     * `fromPhraseNormalized` rejects the row. Callers should follow up with
     * [findByNormalizedPhrase] + [update] when they intend an upsert.
     */
    @Insert(onConflict = OnConflictStrategy.IGNORE)
    suspend fun insert(entry: PersonalizedEntryEntity): Long

    @Query("SELECT * FROM personalized_dictionary WHERE fromPhraseNormalized = :normalized LIMIT 1")
    suspend fun findByNormalizedPhrase(normalized: String): PersonalizedEntryEntity?
