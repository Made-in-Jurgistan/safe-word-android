            ...
            try {
                ...
                if (success) {
                    ...
                    if (attempt < MAX_WORKER_ATTEMPTS) {
                        Timber.w("[DOWNLOAD] doWork | failed, will retry modelId=%s attempt=%d", modelId, attempt)
                        Result.retry()
                    } else {
                        Timber.e(
                            "[DOWNLOAD] doWork | failed and exhausted retries modelId=%s maxAttempts=%d",
                            modelId, MAX_WORKER_ATTEMPTS,
                        )
                        Result.failure()
                    }
                }
                ...
                if (runAttemptCount + 1 < MAX_WORKER_ATTEMPTS) {
                    Timber.d("[DOWNLOAD] doWork | scheduling retry attempt=%d", runAttemptCount + 2)
                    Result.retry()
                } else {
                    Timber.e(
                        "[DOWNLOAD] doWork | exhausted retries modelId=%s maxAttempts=%d",
                        modelId, MAX_WORKER_ATTEMPTS,
                    )
                    Result.failure()
                }
