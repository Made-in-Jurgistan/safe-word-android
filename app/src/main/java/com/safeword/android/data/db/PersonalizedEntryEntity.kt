 ...
 *
 * Indexes:
 *  - `fromPhraseNormalized` is UNIQUE — prevents duplicate substitutions for the
 *    same input phrase regardless of case/spacing differences.
 *  - `enabled` indexed for the hot `observeEnabled()` query.
 *  - `useCount` indexed for ORDER BY useCount DESC.
 */
@Entity(
    tableName = "personalized_dictionary",
    indices = [
        Index(value = ["fromPhraseNormalized"], unique = true),
        Index(value = ["enabled"]),
...
data class PersonalizedEntryEntity(
    ...
    /**
     * Normalized form of [fromPhrase] (lowercased + whitespace collapsed) used for the
     * uniqueness constraint. Repository writes both columns together.
     */
    @ColumnInfo(name = "fromPhraseNormalized")
    val fromPhraseNormalized: String = fromPhrase.normalizePhrase(),
...
/** Normalize a phrase for the uniqueness constraint. */
internal fun String.normalizePhrase(): String =
