 ...
 *
 * Transforms raw ASR text into clean content by resolving disfluencies,
...
object ContentNormalizer {
    ...
    /** Known speech-to-text hallucination phrases (case-insensitive match). */
    ...
    private val HALLUCINATION_PATTERN: Regex = run {
        ...
        // URL-like fragments (www.*, http(s)://*) are common training-data
    ...
    private val CURRENCY_YEN_REWRITE = Regex("(\\d+(?:\\.\\d+)?)\\s+yen\\b", RegexOption.IGNORE_CASE)
    private val CURRENCY_CENT_REWRITE = Regex("(\\d+)\\s+cents?\\b", RegexOption.IGNORE_CASE)

    // -- Fractions ----------------------------------------------------------------

    private val FRACTION_WORDS = mapOf(
        "half" to "½", "a half" to "½",
        "one half" to "½", "one third" to "⅓", "two thirds" to "⅔",
        "one quarter" to "¼", "three quarters" to "¾",
        "one fourth" to "¼", "three fourths" to "¾",
        "one eighth" to "⅛", "three eighths" to "⅜",
        "five eighths" to "⅝", "seven eighths" to "⅞",
    )

    private val FRACTION_PATTERN: Regex = run {
        val alts = FRACTION_WORDS.keys.sortedByDescending { it.length }.joinToString("|") { Regex.escape(it) }
        Regex("(?i)\\b($alts)\\b")
    }
    ...
    /**
     * Matches spoken phone-number patterns like:
     * "555 867 5309", "one eight hundred five five five one two three four",
     * "800-555-1234". Only fires for 7–11 digit spans that look like phone numbers.
     */
    private val PHONE_PREFIX_PATTERN = Regex(
        "(?i)\\b(?:(?:one\\s+)?(?:eight hundred|(?:800|888|877|866|855|844|833))|\\d{3})[\\s.-]+\\d{3}[\\s.-]+\\d{4}\\b"
    )
    ...
    // "January first" → "January 1st", "December twenty fifth" → "December 25th"
    private val DATE_PATTERN: Regex = run {
        val months = MONTH_WORDS.keys.joinToString("|") { Regex.escape(it) }
        val ordinals = ORDINAL_WORDS.keys.sortedByDescending { it.length }.joinToString("|") { Regex.escape(it) }
        Regex("(?i)\\b($months)\\s+($ordinals)\\b")
    }

    // -- URL / email dictation ---------------------------------------------------

    /** "dot com", "dot org", "dot net", etc. → ".com", ".org", ".net" */
    private val DOT_TLD_PATTERN = Regex("(?i)\\b(?:dot|point)\\s+(com|org|net|edu|gov|io|co|app|dev|ai)\\b")
    /** "at sign" between words → @ (for email dictation cleanup) */
    private val EMAIL_AT_PATTERN = Regex("(?<=\\w)\\s+at\\s+(?=\\w+\\s*(?:dot|\\.)\\s*(?:com|org|net|edu|gov|io|co))", RegexOption.IGNORE_CASE)

    // -- Technical dictation: code symbols, file paths, programming ---------------

    /**
     * Spoken code symbol → symbol character.
     * Keys are lowercase. Matched as whole words in order of decreasing length.
     */
    private val SPOKEN_CODE_SYMBOL_MAP: Map<String, String> = mapOf(
        // Brackets & braces
        "open curly brace" to "{", "close curly brace" to "}",
        "open brace" to "{", "close brace" to "}",
        "open bracket" to "[", "close bracket" to "]",
        "open square bracket" to "[", "close square bracket" to "]",
        "open paren" to "(", "close paren" to ")",
        "open parenthesis" to "(", "close parenthesis" to ")",
        "left paren" to "(", "right paren" to ")",
        "left bracket" to "[", "right bracket" to "]",
        "left brace" to "{", "right brace" to "}",
        "left angle bracket" to "<", "right angle bracket" to ">",
        "less than" to "<", "greater than" to ">",

        // Comparison / assignment
        "equals sign" to "=", "double equals" to "==", "triple equals" to "===",
        "not equals" to "!=", "not equal" to "!=",
        "less than or equal" to "<=", "greater than or equal" to ">=",
        "arrow" to "->", "fat arrow" to "=>", "double arrow" to "=>",

        // Operators
        "plus sign" to "+", "minus sign" to "-",
        "plus equals" to "+=", "minus equals" to "-=",
        "times equals" to "*=", "divide equals" to "/=",
        "plus plus" to "++", "minus minus" to "--",
        "ampersand" to "&", "double ampersand" to "&&",
        "pipe" to "|", "double pipe" to "||",
        "caret" to "^", "tilde" to "~",
        "modulo" to "%", "modulus" to "%",

        // Punctuation
        "semicolon" to ";", "colon" to ":",
        "double colon" to "::", "dot" to ".",
        "underscore" to "_", "double underscore" to "__",
        "backtick" to "`", "triple backtick" to "```",
        "backslash" to "\\", "forward slash" to "/",
        "double slash" to "//", "triple slash" to "///",
        "hash sign" to "#", "double hash" to "##",
        "dollar sign" to "\$", "at sign" to "@",
        "exclamation mark" to "!", "bang" to "!",
        "question mark" to "?", "double question" to "??",

        // String delimiters
        "single quote" to "'", "double quote" to "\"",
        "backtick" to "`",

        // Null/optional
        "null" to "null", "nil" to "nil",
        "question dot" to "?.", "bang bang" to "!!",
        "elvis" to "?:",

        // File path
        "slash" to "/", "path separator" to "/",
    )

    private val SPOKEN_CODE_SYMBOL_PATTERN: Regex = run {
        val alts = SPOKEN_CODE_SYMBOL_MAP.keys.sortedByDescending { it.length }.joinToString("|") { Regex.escape(it) }
        Regex("(?i)\\b($alts)\\b")
    }

    /**
     * Spoken programming constructs → code idiom.
     * Converts "camel case" spoken separators into programming naming conventions.
     */
    private val CAMEL_CASE_PATTERN = Regex("(?i)\\b(\\w+)\\s+camel\\s+case\\s+(\\w+)\\b")
    private val SNAKE_CASE_PATTERN = Regex("(?i)\\b(\\w+)\\s+snake\\s+case\\s+(\\w+)\\b")
    private val KEBAB_CASE_PATTERN = Regex("(?i)\\b(\\w+)\\s+kebab\\s+case\\s+(\\w+)\\b")
    ...
    /** Spoken-form contractions expanded before all other normalization steps. */
    private val CONTRACTIONS: List<Pair<Regex, String>> = listOf(
        ...
        Regex("\\btryna\\b", RegexOption.IGNORE_CASE) to "trying to",
        Regex("\\bhafta\\b", RegexOption.IGNORE_CASE) to "have to",
    )

    private fun expandContractions(text: String): String =
        CONTRACTIONS.fold(text) { t, (regex, replacement) -> regex.replace(t, replacement) }

    // -- Public API -----------------------------------------------------------

    /**
     * Apply all content normalization steps to raw transcription text.
     *
    ...
    fun normalize(
        ...
        // 0. Tier 1 contraction expansion (gonna→going to, wanna→want to, etc.)
        result = expandContractions(result)

        // 1. Strip invisible Unicode characters
        result = INVISIBLE_CHARS.replace(result, "")

        // 2. Hallucination filter — remove known transcription artifacts
        ...
        // 7b. Technical dictation: code symbols, naming conventions
        result = convertSpokenCodeSymbols(result)
        result = convertNamingConventions(result)
    ...
    // -- Technical dictation conversion ----------------------------------------

    private fun convertSpokenCodeSymbols(text: String): String {
        return SPOKEN_CODE_SYMBOL_PATTERN.replace(text) { match ->
            SPOKEN_CODE_SYMBOL_MAP[match.value.lowercase()] ?: match.value
        }
    }

    private fun convertNamingConventions(text: String): String {
        var result = text
        // "foo camel case bar" → "fooBar"
        result = CAMEL_CASE_PATTERN.replace(result) { match ->
            val first = match.groupValues[1].lowercase()
            val second = match.groupValues[2].replaceFirstChar { it.uppercaseChar() }
            "$first$second"
        }
        // "foo snake case bar" → "foo_bar"
        result = SNAKE_CASE_PATTERN.replace(result) { match ->
            "${match.groupValues[1].lowercase()}_${match.groupValues[2].lowercase()}"
        }
        // "foo kebab case bar" → "foo-bar"
        result = KEBAB_CASE_PATTERN.replace(result) { match ->
    ...
    internal fun normalizeNumbers(text: String): String {
        ...
        result = CURRENCY_YEN_REWRITE.replace(result) { "¥${it.groupValues[1]}" }
        result = CURRENCY_CENT_REWRITE.replace(result) { "${it.groupValues[1]}¢" }

        // Fractions: "one half" → "½"
        result = FRACTION_PATTERN.replace(result) { match ->
            FRACTION_WORDS[match.value.lowercase()] ?: match.value
        }

        // Dates: "January first" → "January 1st"
        result = normalizeDates(result)

        // URL/email dictation: "dot com" → ".com", inline "at" → "@"
        result = DOT_TLD_PATTERN.replace(result) { ".${it.groupValues[1].lowercase()}" }
        result = EMAIL_AT_PATTERN.replace(result, "@")

        return result
    }

    private fun normalizeDates(text: String): String {
        return DATE_PATTERN.replace(text) { match ->
            val month = match.groupValues[1].replaceFirstChar { it.uppercaseChar() }
            val ordinalWord = match.groupValues[2].lowercase()
            val ordinalDigit = ORDINAL_WORDS[ordinalWord] ?: ordinalWord
            "$month $ordinalDigit"
        }
    }
        ...
        for (token in tokens) {
            when {
                ...
                token in DIGIT_WORDS -> {
                    current += DIGIT_WORDS[token]?.toLong() ?: 0L
                }
                token in TENS_WORDS -> {
                    current += (TENS_WORDS[token] ?: 0L).toLong()
                }
                token in MULTIPLIER_WORDS -> {
                    val multiplier = MULTIPLIER_WORDS[token] ?: 0L
                    if (multiplier >= 1000) {
