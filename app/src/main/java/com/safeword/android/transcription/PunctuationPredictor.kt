...
object PunctuationPredictor {
    ...
    private val QUESTION_START = Regex(
        "^(who|what|where|when|why|how|which|whom|whose|is|are|was|were|do|does|did|can|" +
            "could|would|should|will|shall|have|has|had|isn't|aren't|wasn't|weren't|" +
            "don't|doesn't|didn't)\\b",
        RegexOption.IGNORE_CASE,
    )

    private val TAG_QUESTION = Regex(
        "\\b(right|correct|isn't it|aren't they|doesn't it|don't they|won't they|" +
            "can't we|isn't that|don't you think|wouldn't you say|yeah|no|huh)\\s*[.!]?\\s*$",
        RegexOption.IGNORE_CASE,
    )
    ...
    private val INTRODUCTORY_PHRASES = Regex(
        "^(however|therefore|furthermore|moreover|meanwhile|nevertheless|consequently|" +
            "additionally|unfortunately|fortunately|honestly|basically|actually|anyway|" +
            "well|so|also|finally|first|second|third|next|then|otherwise|instead|" +
            "similarly|likewise|regardless|apparently|obviously|clearly|certainly|" +
            "indeed|besides|personally|naturally|generally|typically|usually|" +
            "specifically|essentially|overall)\\s",
        RegexOption.IGNORE_CASE,
    )
    ...
    private val NEW_SENTENCE_MARKERS = Regex(
        "\\s(anyway|besides|by the way|in fact|for example|for instance|" +
            "in other words|on the other hand|at the same time|as a result|" +
            "in addition|in conclusion|to be honest|to be fair|the thing is|" +
            "the point is|what I mean is)\\s",
        RegexOption.IGNORE_CASE,
    )
    ...
    private fun insertIntroductoryComma(text: String): String {
        // Split on sentence boundaries and apply the leading-introductory comma per
        // sentence — the original implementation anchored on `^` and only ever fired
        // on the very first sentence of the input.
        val sentences = text.split(SENTENCE_SPLIT)
        if (sentences.size <= 1) {
            return INTRODUCTORY_PHRASES.replace(text) { match ->
                val phrase = match.groupValues[1]
                "$phrase, "
            }
        }
        // Re-split with delimiters preserved so we can reassemble the original text.
        val parts = SENTENCE_SPLIT_KEEP.findAll(text).toList()
        if (parts.isEmpty()) return text
        val out = StringBuilder(text.length + 8)
        for (part in parts) {
            val sentence = part.value
            val rebuilt = INTRODUCTORY_PHRASES.replace(sentence.trimStart()) { match ->
                val phrase = match.groupValues[1]
                "$phrase, "
            }
            // Preserve any leading whitespace that was trimmed.
            val leading = sentence.takeWhile { it.isWhitespace() }
            out.append(leading).append(rebuilt)
        }
        return out.toString()
    }
    ...
    /** Captures each sentence including its trailing punctuation and following whitespace. */
    private val SENTENCE_SPLIT_KEEP = Regex("[^.!?]+(?:[.!?]+|$)\\s*")
