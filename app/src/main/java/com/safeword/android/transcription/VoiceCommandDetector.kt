...
object VoiceCommandDetector {
    ...
    private val COMMAND_MAP: Map<String, VoiceAction> = buildMap {
        ...
        put("delete it", VoiceAction.DeleteSelection)
        put("erase that", VoiceAction.DeleteSelection)
        put("erase this", VoiceAction.DeleteSelection)
        put("erase it", VoiceAction.DeleteSelection)
        put("remove that", VoiceAction.DeleteSelection)
        put("remove this", VoiceAction.DeleteSelection)
        put("remove it", VoiceAction.DeleteSelection)
        put("trash that", VoiceAction.DeleteSelection)
        put("wipe that", VoiceAction.DeleteSelection)
        put("kill that", VoiceAction.DeleteSelection)
        put("kill it", VoiceAction.DeleteSelection)
        put("scrap that", VoiceAction.DeleteSelection)
        put("drop that", VoiceAction.DeleteSelection)
        put("delete last word", VoiceAction.DeleteLastWord)
        put("delete the last word", VoiceAction.DeleteLastWord)
        put("erase last word", VoiceAction.DeleteLastWord)
        put("erase the last word", VoiceAction.DeleteLastWord)
        put("remove last word", VoiceAction.DeleteLastWord)
        put("remove the last word", VoiceAction.DeleteLastWord)
        put("delete previous word", VoiceAction.DeleteLastWord)
        put("delete last sentence", VoiceAction.DeleteLastSentence)
        put("delete the last sentence", VoiceAction.DeleteLastSentence)
        put("delete sentence", VoiceAction.DeleteLastSentence)
        put("erase last sentence", VoiceAction.DeleteLastSentence)
        put("erase the last sentence", VoiceAction.DeleteLastSentence)
        put("backspace", VoiceAction.Backspace)
        put("hit backspace", VoiceAction.Backspace)
        put("press backspace", VoiceAction.Backspace)
        put("delete last paragraph", VoiceAction.DeleteLastParagraph)
        put("delete the last paragraph", VoiceAction.DeleteLastParagraph)
        put("erase last paragraph", VoiceAction.DeleteLastParagraph)
        put("erase the last paragraph", VoiceAction.DeleteLastParagraph)

        // Undo / redo
        put("undo", VoiceAction.Undo)
        put("undo that", VoiceAction.Undo)
        put("undo it", VoiceAction.Undo)
        put("undo last", VoiceAction.Undo)
        put("undo last action", VoiceAction.Undo)
        put("scratch that", VoiceAction.Undo)
        put("take that back", VoiceAction.Undo)
        put("revert that", VoiceAction.Undo)
        put("revert", VoiceAction.Undo)
        put("reverse that", VoiceAction.Undo)
        put("redo", VoiceAction.Redo)
        put("redo that", VoiceAction.Redo)
        put("redo it", VoiceAction.Redo)
        put("restore", VoiceAction.Redo)
        put("restore that", VoiceAction.Redo)
        ...
        put("line break", VoiceAction.NewLine)
        put("linefeed", VoiceAction.NewLine)
        put("line feed", VoiceAction.NewLine)
        put("go to next line", VoiceAction.NewLine)
        put("break line", VoiceAction.NewLine)
        ...
        put("start new paragraph", VoiceAction.NewParagraph)
        put("break paragraph", VoiceAction.NewParagraph)
        ...
        put("stop now", VoiceAction.StopListening)
        put("stop dictation", VoiceAction.StopListening)
        put("end dictation", VoiceAction.StopListening)
        put("end recording", VoiceAction.StopListening)
        put("finish", VoiceAction.StopListening)
        put("finished", VoiceAction.StopListening)
        put("finish that", VoiceAction.StopListening)
        put("finish dictation", VoiceAction.StopListening)
        put("commit", VoiceAction.StopListening)
        put("commit that", VoiceAction.StopListening)
        put("commit it", VoiceAction.StopListening)
        put("commit this", VoiceAction.StopListening)
        put("commit now", VoiceAction.StopListening)
        put("done", VoiceAction.StopListening)
        put("that's it", VoiceAction.StopListening)
        put("that's all", VoiceAction.StopListening)
        put("open settings", VoiceAction.OpenSettings)
        put("show settings", VoiceAction.OpenSettings)
        put("dictation settings", VoiceAction.OpenSettings)
        put("safe word settings", VoiceAction.OpenSettings)
        put("open preferences", VoiceAction.OpenSettings)
        ...
        put("hide the keyboard", VoiceAction.Dismiss)
    }
     ...
     *
     * Operands use non-greedy quantifiers so a literal "with" / "to" inside the X
     * operand isn't swallowed by `(.+)` before the connector. Word boundaries on
     * the connector keep `replace within with without` from matching incorrectly.
     */
    ...
    /** Fuzzy replace-verb tolerances (ASR commonly drops 're-' prefix or hears 'place'/'displace'). */
    private val FUZZY_REPLACE_VERBS = setOf("replace", "place", "replaces", "replays", "displace")
    /** Fuzzy swap-verb tolerances. */
    private val FUZZY_SWAP_VERBS = setOf("swap", "swat", "swaps", "switch", "switches", "switched")
    /** Fuzzy connectors that may stand in for "with"/"to". */
    private val FUZZY_REPLACE_CONNECTORS = setOf("with", "to", "and", "for", "by", "into")

    /**
     * Fuzzy fallback for replace/swap commands when strict regex fails.
     * Recognises e.g. "place hello with world", "swap hello and world".
     * Returns null if the structure isn't unambiguously a 2-argument replace.
     */
    private fun matchFuzzyReplace(normalized: String): VoiceAction? {
        val tokens = normalized.split(' ')
        if (tokens.size < 4) return null
        val verb = tokens[0]
        val isReplace = verb in FUZZY_REPLACE_VERBS
        val isSwap = verb in FUZZY_SWAP_VERBS
        if (!isReplace && !isSwap) return null
        // Find a connector token that splits the args roughly evenly (avoid taking
        // the first occurrence which may be inside the "old" phrase).
        var bestIdx = -1
        var bestDist = Int.MAX_VALUE
        val mid = tokens.size / 2
        for (i in 1 until tokens.size - 1) {
            if (tokens[i] in FUZZY_REPLACE_CONNECTORS) {
        ...
        if (bestIdx <= 1 || bestIdx >= tokens.size - 1) return null
        val oldText = tokens.subList(1, bestIdx).joinToString(" ").trim()
        val newText = tokens.subList(bestIdx + 1, tokens.size).joinToString(" ").trim()
    ...
    fun detect(rawText: String, fieldType: FieldType = FieldType.UNKNOWN): VoiceCommandResult {
        ...
        if (exactAction != null) {
            ...
            if (gated != null) {
                Timber.i(
                    "[VOICE] VoiceCommandDetector.detect | exact-command=\"%s\" " +
                        "action=%s fieldType=%s",
                    normalized, gated, fieldType,
                )
        ...
        if (customAction != null) {
            ...
            if (gated != null) {
                Timber.i(
                    "[VOICE] VoiceCommandDetector.detect | custom-command=\"%s\" " +
                        "action=%s fieldType=%s",
                    normalized, gated, fieldType,
                )
                return VoiceCommandResult.Command(gated)
        ...
        if (intentResult != null) {
            ...
            if (gated != null) {
                Timber.i(
                    "[VOICE] VoiceCommandDetector.detect | intent-command=\"%s\" " +
                        "action=%s confidence=%.3f fieldType=%s",
        ...
        if (fuzzyAction != null) {
            ...
            if (gated != null) {
                Timber.i(
                    "[VOICE] VoiceCommandDetector.detect | fuzzy-command=\"%s\" " +
                        "action=%s fieldType=%s",
                    normalized, gated, fieldType,
                )
                return VoiceCommandResult.Command(gated)
    ...
    /**
     * Token-prefix index: maps the first 3 chars of any token in a command key
     * to the set of candidate keys containing that prefix. Used to prune the
     * fuzzy-match search space from ~150 keys to typically 5–15.
     */
    private val tokenPrefixIndex: Map<String, List<String>> by lazy {
        val map = HashMap<String, MutableList<String>>(256)
        for (key in COMMAND_MAP.keys) {
            for (token in key.split(' ')) {
                if (token.length < 3) continue
                val prefix = token.take(3)
                map.getOrPut(prefix) { mutableListOf() }.add(key)
            }
        }
        map
    }
     ...
     *
     * Uses a token-prefix index to restrict candidates to keys that share at least
     * one 3-char token prefix with the input — typical reduction from ~150 keys to ~5–15.
     */
    private fun tryFuzzyMatch(normalized: String): VoiceAction? {
        ...
        // Collect candidate keys via the token-prefix index.
        val candidateKeys = LinkedHashSet<String>(32)
        for (token in tokens) {
            if (token.length < 3) continue
            tokenPrefixIndex[token.take(3)]?.let { candidateKeys.addAll(it) }
        }
    ...
    private fun matchParameterizedCommand(normalized: String): VoiceAction? {
        ...
        // Strict regex didn't match — try the fuzzy replace fallback for ASR-mangled verbs.
        matchFuzzyReplace(normalized)?.let { return it }
    ...
    private fun logPotentialCommandMiss(trimmedText: String) {
        ...
        if (hasCommandStem) {
            Timber.w(
                "[VOICE] VoiceCommandDetector.detect | near-miss raw=\"%s\" normalized=\"%s\"",
