...
object ConfusionSetCorrector {
    ...
    fun applyInContext(text: String, context: Context): String {
        ...
        var result = applyToTooTwoHeuristic(text)
        for (rule in rules) {
    ...
    /**
     * Very small context window heuristics for the "to / too / two" confusion set.
     *
     * This runs before the generic rules so it can correct common patterns like:
     * - "me to" -> "me too"
     * - "I have to many" -> "I have too many"
     *
     * It does not attempt full POS tagging; it only targets high-precision patterns.
     */
    private fun applyToTooTwoHeuristic(text: String): String {
        val rawTokens = text.split(Regex("\\s+")).filter { it.isNotEmpty() }
        if (rawTokens.size < 2) return text

        val tokens = rawTokens.toMutableList()
        for (i in tokens.indices) {
            val (prefix, core, suffix) = splitToken(tokens[i])
            val lower = core.lowercase()
            if (lower != "to" && lower != "too" && lower != "two") continue

            val prevCore = if (i > 0) splitToken(tokens[i - 1]).second.lowercase() else ""
            val nextCore = if (i + 1 < tokens.size) splitToken(tokens[i + 1]).second.lowercase() else ""

            val replacement = when (lower) {
                "to" -> when {
                    prevCore in PRONOUNS && nextCore.isEmpty() -> "too" // "me to" at end
                    // "me to also" style
                    prevCore in PRONOUNS && nextCore.isNotEmpty() && nextCore !in COMMON_VERBS -> "too"
                    nextCore in TOO_NEXT_WORDS -> "too" // "to many" -> "too many"
                    else -> null
                }
                "too" -> when {
                    nextCore in COMMON_VERBS -> "to" // "too go" -> "to go"
                    else -> null
                }
                "two" -> null
                else -> null
            ...
            if (replacement != null) {
                tokens[i] = prefix + preserveCase(core, replacement) + suffix
            }
        }
    ...
    private fun splitToken(token: String): Triple<String, String, String> {
        val start = token.indexOfFirst { it.isLetterOrDigit() || it == '\'' }
        if (start == -1) return Triple("", token, "")
        val end = token.indexOfLast { it.isLetterOrDigit() || it == '\'' }
    ...
    private val PRONOUNS = setOf("me", "you", "him", "her", "them", "us")
    private val TOO_NEXT_WORDS = setOf("many", "much", "far", "soon", "late")
    private val COMMON_VERBS = setOf(
        "be", "do", "go", "have", "get", "make", "take", "say", "see", "use",
        "try", "tell", "ask", "work", "call", "need", "feel", "leave", "put",
        "keep", "let", "help", "start", "stop", "run", "move", "write", "read",
        "send", "open", "close", "save", "copy", "paste", "delete",
    )
