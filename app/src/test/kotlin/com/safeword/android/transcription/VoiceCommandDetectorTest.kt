...
class VoiceCommandDetectorTest {
    ...
    // -------------------------------------------------------------------------
    // Word-targeting commands — select / highlight
    // -------------------------------------------------------------------------

    @Test
    fun `select hello returns SelectWord`() {
        val result = VoiceCommandDetector.detect("select hello")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectWord("hello"), result.action)
    }

    @Test
    fun `highlight world returns SelectWord`() {
        val result = VoiceCommandDetector.detect("highlight world")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectWord("world"), result.action)
    }

    @Test
    fun `select the word example returns SelectWord`() {
        val result = VoiceCommandDetector.detect("select the word example")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectWord("example"), result.action)
    }

    @Test
    fun `highlight the word test returns SelectWord`() {
        val result = VoiceCommandDetector.detect("highlight the word test")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectWord("test"), result.action)
    }

    @Test
    fun `select all still returns SelectAll not SelectWord`() {
        val result = VoiceCommandDetector.detect("select all")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectAll, result.action)
    }
    ...
    @Test
    fun `select last word still returns SelectLastWord not SelectWord`() {
        val result = VoiceCommandDetector.detect("select last word")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SelectLastWord, result.action)
    }
    ...
    // -------------------------------------------------------------------------
    // Word-targeting commands — copy
    // -------------------------------------------------------------------------

    @Test
    fun `copy hello returns CopyWord`() {
        val result = VoiceCommandDetector.detect("copy hello")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.CopyWord("hello"), result.action)
    }

    @Test
    fun `copy the word example returns CopyWord`() {
        val result = VoiceCommandDetector.detect("copy the word example")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.CopyWord("example"), result.action)
    }

    @Test
    fun `copy that still returns Copy not CopyWord`() {
        val result = VoiceCommandDetector.detect("copy that")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.Copy, result.action)
    }
    ...
    // -------------------------------------------------------------------------
    // Word-targeting commands — delete
    // -------------------------------------------------------------------------

    @Test
    fun `delete hello returns DeleteWord`() {
        val result = VoiceCommandDetector.detect("delete hello")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteWord("hello"), result.action)
    }

    @Test
    fun `erase the word example returns DeleteWord`() {
        val result = VoiceCommandDetector.detect("erase the word example")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteWord("example"), result.action)
    }

    @Test
    fun `remove hello returns DeleteWord`() {
        val result = VoiceCommandDetector.detect("remove hello")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteWord("hello"), result.action)
    }

    @Test
    fun `delete that still returns DeleteSelection not DeleteWord`() {
        val result = VoiceCommandDetector.detect("delete that")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteSelection, result.action)
    }
    ...
    // -------------------------------------------------------------------------
    // Replace/switch commands
    // -------------------------------------------------------------------------

    @Test
    fun `switch hello with world returns ReplaceText`() {
        val result = VoiceCommandDetector.detect("switch hello with world")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.ReplaceText("hello", "world"), result.action)
    }

    @Test
    fun `switch hello to world returns ReplaceText`() {
        val result = VoiceCommandDetector.detect("switch hello to world")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.ReplaceText("hello", "world"), result.action)
    }
    ...
    @Test
    fun `spell a b c returns SpellOut abc`() {
        val result = VoiceCommandDetector.detect("spell a b c")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SpellOut("abc"), result.action)
    }

    @Test
    fun `spell single letter returns SpellOut`() {
        val result = VoiceCommandDetector.detect("spell z")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SpellOut("z"), result.action)
    }

    @Test
    fun `spell NATO alphabet returns SpellOut`() {
        val result = VoiceCommandDetector.detect("spell alpha bravo charlie")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SpellOut("abc"), result.action)
    }

    @Test
    fun `spell mixed letters and NATO returns SpellOut`() {
        val result = VoiceCommandDetector.detect("spell a bravo c")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SpellOut("abc"), result.action)
    }

    @Test
    fun `type out h e l l o returns SpellOut`() {
        val result = VoiceCommandDetector.detect("type out h e l l o")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.SpellOut("hello"), result.action)
    }

    @Test
    fun `spell with invalid token falls through`() {
        val result = VoiceCommandDetector.detect("spell banana")
        assertIs<VoiceCommandResult.Text>(result)
    }
    ...
    @Test
    fun `letter a returns InsertText a`() {
        val result = VoiceCommandDetector.detect("letter a")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.InsertText("a"), result.action)
    }

    @Test
    fun `type letter b returns InsertText b`() {
        val result = VoiceCommandDetector.detect("type letter b")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.InsertText("b"), result.action)
    }

    @Test
    fun `the letter z returns InsertText z`() {
        val result = VoiceCommandDetector.detect("the letter z")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.InsertText("z"), result.action)
    }

    // -------------------------------------------------------------------------
    // New natural-language phrasings added in STT+command overhaul
    // -------------------------------------------------------------------------

    @Test
    fun `kill it returns DeleteSelection`() {
        val result = VoiceCommandDetector.detect("kill it")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteSelection, result.action)
    }

    @Test
    fun `wipe that returns DeleteSelection`() {
        val result = VoiceCommandDetector.detect("wipe that")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteSelection, result.action)
    }

    @Test
    fun `trash that returns DeleteSelection`() {
        val result = VoiceCommandDetector.detect("trash that")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteSelection, result.action)
    }

    @Test
    fun `delete the last word returns DeleteLastWord`() {
        val result = VoiceCommandDetector.detect("delete the last word")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.DeleteLastWord, result.action)
    }

    @Test
    fun `revert returns Undo`() {
        val result = VoiceCommandDetector.detect("revert")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.Undo, result.action)
    }

    @Test
    fun `line break returns NewLine`() {
        val result = VoiceCommandDetector.detect("line break")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.NewLine, result.action)
    }

    @Test
    fun `break paragraph returns NewParagraph`() {
        val result = VoiceCommandDetector.detect("break paragraph")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.NewParagraph, result.action)
    }

    @Test
    fun `stop now returns StopListening`() {
        val result = VoiceCommandDetector.detect("stop now")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.StopListening, result.action)
    }

    @Test
    fun `thats it returns StopListening`() {
        val result = VoiceCommandDetector.detect("that's it")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.StopListening, result.action)
    }

    @Test
    fun `show settings returns OpenSettings`() {
        val result = VoiceCommandDetector.detect("show settings")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.OpenSettings, result.action)
    }

    @Test
    fun `hide keyboard returns Dismiss`() {
        val result = VoiceCommandDetector.detect("hide keyboard")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.Dismiss, result.action)
    }

    @Test
    fun `press backspace returns Backspace`() {
        val result = VoiceCommandDetector.detect("press backspace")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.Backspace, result.action)
    }

    // -------------------------------------------------------------------------
    // Fuzzy replace-verb tolerance (ASR-mangled verb at sentence start)
    // -------------------------------------------------------------------------

    @Test
    fun `place hello with world fuzzy-matches ReplaceText`() {
        // ASR commonly drops "re-" prefix — "place" → "replace"
        val result = VoiceCommandDetector.detect("place hello with world")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.ReplaceText("hello", "world"), result.action)
    }

    @Test
    fun `swap hello and world accepts 'and' connector`() {
        val result = VoiceCommandDetector.detect("swap hello and world")
        assertIs<VoiceCommandResult.Command>(result)
        assertEquals(VoiceAction.ReplaceText("hello", "world"), result.action)
    }

    @Test
    fun `switch to airplane mode is not a fuzzy replace`() {
        // Connector is at index 1 → too early, guard rejects.
        val result = VoiceCommandDetector.detect("switch to airplane mode")
        assertIs<VoiceCommandResult.Text>(result)
    }
}
