...
Safe Word is an on-device voice-to-text Android app. All speech inference (Moonshine streaming)
and voice activity detection (Silero VAD via ONNX Runtime and optional native GGML Silero VAD) run locally — no audio or text
...
│   │   ├── MoonshineStreamingEngine.kt # Moonshine v2 streaming model lifecycle
│   │   ├── MoonshineEventMapper.kt  # SDK event mapping into app states
...
│   └── native/                 # Native dependencies and glue code
...
AudioRecorder (16 kHz PCM)
  ...
  → TranscriptionCoordinator (orchestrates everything)
    ...
    ├── MoonshineStreamingEngine (streaming decode loop)
    │   ├── Model load from app-private storage
    │   ├── LineTextChanged events for live transcript UI
    │   ├── LineCompleted events for command detection + insertion
    │   └── Single backend path (no legacy batch fallback)
...
- **Native threads**: Moonshine SDK manages internal inference threads

`TranscriptionCoordinator` serialises backend lifecycle operations and line handling work.
Do not run concurrent load/start/stop sequences against the same streaming engine instance.
...
- Native/SDK boundaries: return safe fallback results on failure, never crash UI paths
...
- Functions/properties: camelCase (`optimalInferenceThreads`)
...
4. `nativeFree(context)` → Release native context
...
- Thread count clamped to `[1, 16]`, set by `InferenceConfig.optimalInferenceThreads()`
...
Native context pointers are **NOT thread-safe** for concurrent calls. The Kotlin-side
`TranscriptionCoordinator` serialises backend operations. Never call native methods
...
| `nativeVadEnabled`        | true      | Native GGML Silero VAD path enabled    |
...
| `noSpeechThreshold`       | 0.6       | ASR no-speech suppression threshold    |
| `logprobThreshold`        | -1.0      | ASR average logprob suppression        |
| `entropyThreshold`        | 2.4       | ASR entropy-based suppression          |
...
5. **JNI thread safety** — Never call native inference JNI concurrently on the same context pointer.
